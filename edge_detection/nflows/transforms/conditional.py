...
from nflows.transforms.orthogonal import ParametrizedHouseHolder, _apply_batchwise_transforms_nodet
from nflows.transforms.adaptive_sigmoids import SumOfSigmoids
from typing import *
...
class ConditionalScaleTransform(ConditionalTransform):
    ...
    def _forward_given_params(self, inputs, conditional_params):
        unconstrained_scale = self._unconstrained_scale(
            conditional_params
        )
        scale = torch.nn.functional.softplus(unconstrained_scale) + 1e-4
        outputs = inputs * scale.view(inputs.shape)
        logabsdet = torch.log(scale).view(inputs.shape[0])
        return outputs, logabsdet
    ...
    def _unconstrained_scale(self, conditional_params):
        # split_idx = autoregressive_params.size(1) // 2
        # unconstrained_scale = autoregressive_params[..., :split_idx]
        # shift = autoregressive_params[..., split_idx:]
        # return unconstrained_scale, shift
...
class ConditionalLUTransform(ConditionalTransform):
    ...
    def _create_lower_upper(self, conditional_params):
        ...
        lower = torch.exp(self.scale_non_diag) * torch.tril(unconstrained_matrix_vals, diagonal=-1) + self.diag_entries
        upper_diag = torch.diag_embed(self.softplus(unconstrained_matrix_vals.diagonal(0, -1, -2)) + self.eps)
        upper = torch.exp(self.scale_non_diag) * torch.triu(unconstrained_matrix_vals, diagonal=1) + upper_diag
        return lower, upper
...
class ConditionalPlanarTransform(ConditionalTransform):
    ...
    def _forward_given_params(self, inputs, conditional_params):
        ...
        logabsdet = torch.log(1e-4 + abs_det).squeeze()
        return outputs, logabsdet
