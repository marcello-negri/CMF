...
def plot_marginal_log_likelihood(lambda_sorted, kl, T, conf=0.95, file_name=None, folder_name=None):
    ...
    plt.legend()
    plt.savefig(f"{folder_name}W_marginal_likelihood_{T}.pdf", bbox_inches='tight')
    plt.clf()
    # plt.show()
...
def plot_W_comparison_(W_flow, W_sklearn, lambdas, p, T, conf=0.95, off_diagonal=True, folder_name=None, n_plots=3):
